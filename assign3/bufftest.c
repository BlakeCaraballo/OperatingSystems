...
int main(int argc, char *argv[])
{
    ...
    int *start_num_ptr = &START_NUMBER; // create a pointer to START_NUMBER
    ...
    for(i = 0; i < producerThreads; i++)
    {
        ...
        pthread_create(&tid, &attr, producer, start_num_ptr); // pass the pointer as a parameter
        (*start_num_ptr)++; // increment the value pointed to by the pointer
    }
    ...
    for(j = 0; j < consumerThreads; j++)
    {
        ...
        pthread_attr_t attr;
        pthread_attr_init(&attr);
        pthread_create(&tid, &attr, consumer, start_num_ptr); // pass the pointer as a parameter
        (*start_num_ptr)++; // increment the value pointed to by the pointer
    }
...
void *producer(void *param)
{
    int* start_num_ptr = (int*) param; // cast the parameter back to an int pointer
    ...
    while(TRUE)
    {
        ...
        printf("Producer %ld produced %d\n", pthread_self(), (*start_num_ptr)); // print the current value of START_NUMBER
        (*start_num_ptr)++; // increment the value pointed to by the pointer
    }
}
